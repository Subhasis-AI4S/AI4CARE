...
import { SuggestiveQaIntake } from '../components/SuggestiveQaIntake';
...
export const NewSession = () => {
    ...
    const handleComplaintSubmit = async () => {
        ...
        try {
            ...
            // Move directly to Step 3 — Suggestive Card Q&A (no AI call needed)
            resetTranscript();
            setStep(3);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="p-4 md:p-8 md:pt-12 max-w-[800px] mx-auto h-full flex flex-col space-y-8">
            ...
            <div className="glass-card rounded-[2.5rem] p-8 flex-1 flex flex-col border border-border/50">
                ...
                {/* STEP 3: SUGGESTIVE CARD Q&A (Respiratory Clinical Intake) */}
                {step === 3 && (
                    <div className="flex-1 animate-in fade-in flex flex-col -m-2">
                        <div className="flex items-center justify-between mb-3 px-2">
                            <div>
                                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Clinical Intake</h2>
                                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Respiratory symptom assessment — select the best matching options</p>
                            </div>
                            <div className="bg-teal-500/10 text-teal-600 dark:text-teal-400 text-xs font-bold px-3 py-1.5 rounded-full border border-teal-200 dark:border-teal-800 uppercase tracking-widest">
                                🫁 Respiratory
                            </div>
                        </div>
                        <SuggestiveQaIntake
