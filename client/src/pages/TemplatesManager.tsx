    ...
    return (
        <div className="p-8 max-w-6xl mx-auto h-full flex flex-col">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">{t('templates')}</h1>
                    <p className="text-slate-500 mt-1">{t('templates_flow.subtitle')}</p>
                </div>
                ...
                <div className="p-4 border-b border-slate-200 bg-slate-50">
                    <div className="relative max-w-md">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredTemplates.map(tmpl => {
                            const qs = JSON.parse(tmpl.questions || '[]');
                            return (
                                <div key={tmpl.id} className="border border-slate-200 rounded-xl p-5 hover:border-teal-300 transition-all hover:shadow-md bg-white group flex flex-col">
                                    ...
                                    <div className="mb-4">
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">{t('templates_flow.trigger_keywords')}</p>
                                        <p className="text-sm text-slate-700 line-clamp-2">{tmpl.trigger_keywords}</p>
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">{t('templates_flow.questions')} ({qs.length})</p>
                                        <ul className="text-sm text-slate-600 space-y-1 pl-4 list-disc line-clamp-3">
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-xl">
                <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-2xl">
                    <h2 className="text-xl font-bold text-slate-800">{template.id ? t('templates_flow.edit_template') : t('templates_flow.new_template')}</h2>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-600 p-1"><X className="w-6 h-6" /></button>
                </div>
                <div className="p-6 overflow-y-auto flex-1 space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">{t('templates_flow.label_name')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">{t('templates_flow.label_keywords')}</label>
                    ...
                    <div>
                        <div className="flex justify-between items-end mb-2">
                            <label className="block text-sm font-medium text-slate-700">{t('templates_flow.label_questions')}</label>
                ...
                <div className="p-6 border-t border-slate-200 bg-slate-50 flex justify-end space-x-3 rounded-b-2xl">
                    <button onClick={onClose} className="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-200 rounded-xl transition-colors">{t('common.cancel')}</button>
