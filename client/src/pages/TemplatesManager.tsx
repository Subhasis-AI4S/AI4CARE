    ...
    return (
        <div className="p-8 max-w-6xl mx-auto h-full flex flex-col">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-text">{t('templates')}</h1>
                    <p className="text-text-muted mt-1">{t('templates_flow.subtitle')}</p>
                </div>
                ...
                <div className="p-4 border-b border-border bg-background">
                    <div className="relative max-w-md">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 bg-background text-text"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredTemplates.map(tmpl => {
                            ...
                            return (
                                <div key={tmpl.id || Math.random()} className="border border-border rounded-xl p-5 hover:border-teal-300 transition-all hover:shadow-md bg-background group flex flex-col">
                                    <div className="flex justify-between items-start mb-3">
                                        <h3 className="font-bold text-lg text-text">{tmpl.name || 'Unnamed Template'}</h3>
                                    ...
                                    <div className="mb-4">
                                        <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1">{t('templates_flow.trigger_keywords')}</p>
                                        <p className="text-sm text-text line-clamp-2">{tmpl.trigger_keywords || 'No keywords'}</p>
                                    </div>
                                    <div className="flex-1">
                                        <ul className="text-sm text-text space-y-1 pl-4 list-disc line-clamp-3">
...
const TemplateModal = ({ template, onClose, onSave }: any) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-surface rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-xl">
                <div className="p-6 border-b border-border flex justify-between items-center bg-background rounded-t-2xl">
                    <h2 className="text-xl font-bold text-text">{template.id ? t('templates_flow.edit_template') : t('templates_flow.new_template')}</h2>
                    <button onClick={onClose} className="text-text-muted hover:text-text p-1"><X className="w-6 h-6" /></button>
                </div>
                <div className="p-6 overflow-y-auto flex-1 space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-text mb-1">{t('templates_flow.label_name')}</label>
                        <input value={name} onChange={e => setName(e.target.value)} type="text" className="w-full px-4 py-2 border border-border bg-background text-text rounded-lg focus:ring-2 focus:ring-teal-500" placeholder={t('templates_flow.placeholder_name')} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-text mb-1">{t('templates_flow.label_keywords')}</label>
                        <input value={keywords} onChange={e => setKeywords(e.target.value)} type="text" className="w-full px-4 py-2 border border-border bg-background text-text rounded-lg focus:ring-2 focus:ring-teal-500" placeholder={t('templates_flow.placeholder_keywords')} />
                    </div>
                    <div>
                            ...
                            {questions.map((q, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    ...
                                    <input 
                                        ...
                                        type="text" 
                                        className="flex-1 px-4 py-2 border border-border bg-background text-text rounded-lg focus:ring-2 focus:ring-teal-500" 
                ...
                <div className="p-6 border-t border-border bg-background flex justify-end space-x-3 rounded-b-2xl">
                    <button onClick={onClose} className="px-5 py-2.5 text-text font-medium hover:bg-surface rounded-xl transition-colors">{t('common.cancel')}</button>
